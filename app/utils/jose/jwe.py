...
def encrypt(
    ...
    Args:
        plaintext (str|bytes): A bytes object to encrypt
        key (str | bytes | dict): The key(s) to use for encrypting the content. Can be
    ...
    Examples:
        >>> encrypt('Hello, World!', 'asecret128bitkey', algorithm='dir', encryption='A128GCM')
...
def decrypt(jwe_str: Union[str, bytes], key: Union[str, bytes, dict]):
    """Decrypts a JWE compact serialized string and returns the plaintext.

    Args:
        jwe_str (bytes): A JWE to be decrypt.
        key (str | bytes | dict): A key to attempt to decrypt the payload with. Can be
            individual JWK or JWK set.
    ...
    Examples:
        >>> decrypt(jwe_str, 'asecret128bitkey')
        'Hello, World!'
    """
...
def get_unverified_header(jwe_str: Union[str, bytes]) -> Mapping:
    """Returns the decoded headers without verification of any kind.

    Args:
        jwe_str (str|bytes): A compact serialized JWE to decode the headers from.
...
def _decrypt_and_auth(
        cek_bytes: bytes, enc: str, cipher_text: bytes, iv: bytes, aad: bytes, auth_tag: bytes
) -> bytes:
    """
...
def _jwe_compact_deserialize(jwe_bytes: Union[str, bytes]) -> Tuple[Mapping, bytes, bytes, bytes, bytes, bytes]:
    """
    Deserialize and verify the header and segments are appropriate.

    Args:
        jwe_bytes (str|bytes): The compact serialized JWE
    Returns:
...
def _encoded_header(alg: str, enc: str, zip: str, cty: str, kid: str) -> bytes:
    """
...
def _encrypt_and_auth(key: "Key", alg: str, enc: str, zip: str, plaintext: bytes, aad: bytes):
    """
...
def _decompress(zip: str, compressed: bytes) -> bytes:
    """
...
def _get_cek(enc: str, alg: str, key: "Key") -> Tuple[bytes, bytes]:
    """
...
def _get_direct_key_wrap_cek(key: "Key") -> Tuple[bytes, bytes]:
    """
    ...
    Return:
        (bytes, bytes): Tuple of (cek Key object and wrapped cek)
    """
...
def _get_key_bytes_from_key(key: "Key") -> bytes:
    """
...
def _get_key_wrap_cek(enc, key) -> Tuple[bytes, bytes]:
...
def _get_random_cek_bytes_for_enc(enc: str) -> bytes:
    """
...
def _auth_tag(ciphertext: bytes, iv: bytes, aad: bytes, mac_key: "Key", tag_length: int):
    """
    ...
    Args:
        ...
        mac_key (Key): Key to use in generating the MAC
...
def _jwe_compact_serialize(
        encoded_header: bytes, encrypted_cek: bytes, iv: bytes, cipher_text: bytes, auth_tag: bytes
) -> bytes:
    """
